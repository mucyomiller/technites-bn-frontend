...
import ProfileComponent, { ProfilePage } from '../../components/profile-page/ProfilePage';
...
describe('profile unit tests', () => {
  beforeEach(() => {
    wrapper = shallow(
      <ProfilePage fetchProfileInfo={jest.fn()} user={user} updateprofileInfo={jest.fn()} />,
    );
  });
  ...
  it('simulate profile  component with errors object', (done) => {
    ...
    wrapper.setProps({ status: 'update_fail', error: { error: 'simple error' } });
...
describe('profile integration tests', () => {
  ...
  it('simulate handleInputChange', () => {
    ...
    expect(wrapper.find('Input[name="firstname"]').props().value).toBe('miller');
  });

  it('mount profile  component with default props', (done) => {
    wrapper.setProps({ status: { message: 'test' } });
    expect(wrapper).toHaveLength(1);
    expect(wrapper.instance().props.status).toHaveProperty('message');
    done();
  });
